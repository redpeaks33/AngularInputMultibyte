...
namespace WordCurationEngine.source
{
    public class WordEngine
    {
        ...
        #region Get Sound Contents

        public string GetSoundContents(HttpRequestMessage Request)
        {
            var word = Request.RequestUri.LocalPath.Split('/')[5];
            var stringBuilder = new StringBuilder();
            stringBuilder.Append(@"select top 1 Sound
	                                    from [WordChunk].[dbo].[Sound] S
	                                    inner join [WordChunk].[dbo].[WordChunk] W
	                                    on S.WordID = W.ID where W.ID = '" + word + "'");
            var dataSet = DBAccessor.ExecuteSQLToGetDataBase(stringBuilder.ToString(), DataBaseType.WORD);

            if (dataSet.Tables[0].Rows.Count > 0)
            {
                var value = (byte[])dataSet.Tables[0].Rows[0][0];

                var soundBinary = new SoundBinary() { soundBinary = value };

                return JsonConvert.SerializeObject(soundBinary, Formatting.None);
            }
            return null;
        }

        #endregion Get Sound Contents

        #region Get Analytic Contents

        public string GetDetailContents(HttpRequestMessage Request)
        {
            var word = Request.RequestUri.LocalPath.Split('/')[5];
            //var dataSet = GetAnalyticDataSet(word);
            var dataSet = GetWordDetailDataSet(word);

            return JsonConvert.SerializeObject(ConvertDetailModel(dataSet), Formatting.None);
        }

        private WordInfo ConvertDetailModel(DataSet dataSet)
        {
            ...
            if (dataSet.Tables.Count != 0)
            {
                ...
                foreach (var row in dataSet.Tables[1].AsEnumerable())
                {
                    ...
                    {
                        meaningID = (row["MeaningID"] is DBNull) ? -1 : (int)row["MeaningID"],
                        regionID = (row["RegionID"] is DBNull) ? -1 : (int)row["RegionID"],
                        meaning = (row["Meaning"] is DBNull) ? "nothing" : ConvertValidMeaningString((string)row["Meaning"]),
                    });
                    wordInfo.meaningInfoList = wordInfo.meaningInfoList.OrderBy(n => n.regionID).ToList();
                }

                wordInfo.existsound = dataSet.Tables[2].Rows.Count > 0;
            }
        ...
        public string GetContents(HttpRequestMessage Request, string type)
        {
            ...
            return JsonConvert.SerializeObject(ConvertWordListModel(dataSet), Formatting.None);
        }
        ...
        private DataSet GetWordChunkDataSet(string word, string type)
        {
            ...
            //           @"select " + searchNum + @"VLevel,Word.ID as WordID,Word,Pronunciation,
            //               IMAGE.ImageID,IMAGE.Url,IMAGE.ThumnailUrl
            //            from [WordChunk].[dbo].[Rank] RANK
            //inner join  [WordChunk].[dbo].[WordChunk] WORD On WORD.ID = RANK.WordID
            ...
            //inner join[WordChunk].[dbo].[Sound]        Sound On WORD.ID = Sound.WordID
        }
        ...
        private string CreateType(string type)
        {
            ...
            switch (type)
            {
                ...
                case "topSearch": searchNum = " top 1000 "; break;
                default: break;
            }
            return searchNum;
        }

        private DataSet GetWordDetailDataSet(string word)
        {
            //var condition = CreateCondition(word);
            var stringBuilder = new StringBuilder();
            stringBuilder.Append(
                        @"select top 10 WORD.ID as WordID,WORD.word,A.ID as ArticleID,A.Title,A.Intro,A.Contents,A.Url,A.ThumnailUrl,A.Time
	                        ...
	                        where WORD.ID = " + word +
                        @"select Word.ID as WordID,Word,Pronunciation,
                            MEAN.ID as MeaningID, MEAN.RegionID,MEAN.ClassID,MEAN.Type, MEAN.Meaning
                            from [WordChunk].[dbo].[WordChunk] WORD
                            inner join [WordChunk].[dbo].[Meaning] MEAN On WORD.ID = MEAN.WordID AND (RegionID = 1 OR (RegionID = 2 AND Type = 2))
                            where WORD.ID = " + word +
        ...
        private string CreateCondition(string word)
        {
            ...
            {
                conditionSQL.Append(" VLevel <= 30 AND Word LIKE '%" + w + "%' OR ");
            }

            return conditionSQL.ToString(0, conditionSQL.Length - 4);
        }

        private List<WordInfo> ConvertWordListModel(System.Data.DataSet dataSet)
        {
            ...
            foreach (var rowGroups in dataSet.Tables[0].AsEnumerable().GroupBy(n => n["WordID"]))
            {
                ...
                var wordInfo = new WordInfo()
                {
                    ...
                    //svl = (row["VLevel"] is DBNull) ? -1 : Convert.ToInt32(row["VLevel"].ToString()),
                    phonetic = (row["Pronunciation"] is DBNull) ? "-" : (string)row["Pronunciation"],
                    level = (row["VLevel"] is DBNull) ? "-" : row["VLevel"].ToString().PadLeft(2, '0'),
                    //soundurl = (row["soundUrl"] is DBNull) ? "" : row["soundUrl"].ToString(),
                };

                foreach (var imageRow in rowGroups)
                {
        ...
        private class SoundBinary
        {
            [JsonProperty(PropertyName = "soundBinary")]
            public byte[] soundBinary { get; set; }
        }
    }
}